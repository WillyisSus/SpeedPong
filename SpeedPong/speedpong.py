...
class minigame():
    ...
    def drawWindow(self):
        ...
        pygame.draw.rect(self.screen, (0,0,0), self.topborder )
        pygame.draw.rect(self.screen, (0,0,0), self.bottomborder )
        pygame.display.update()
    def checkCollision(self):
        if self.ball.rect.colliderect(self.playerX.rect):
            if abs(self.ball.rect.left -  self.playerX.rect.right) <= 20 and self.ball.speedx < 0:
                self.ball.speedx *= ((-1)*self.ball.accelerate)
        if self.ball.rect.colliderect(self.playerO.rect): 
            if abs(self.ball.rect.right -  self.playerO.rect.left) <= 20 and self.ball.speedx > 0:
                self.ball.speedx *= ((-1)*self.ball.accelerate)   
        if self.ball.rect.colliderect(self.playerX.rect):
            if abs(self.ball.rect.top -  self.playerX.rect.bottom) <= 20 and self.ball.speedy > 0:
                self.ball.speedy *= (-1)
        if self.ball.rect.colliderect(self.playerO.rect): 
            if abs(self.ball.rect.top -  self.playerO.rect.bottom) <= 20 and self.ball.speedy > 0:
                self.ball.speedx *= (-1)  
        if self.ball.rect.colliderect(self.playerX.rect):
            if abs(self.ball.rect.bottom -  self.playerX.rect.top) <= 20 and self.ball.speedy < 0:
                self.ball.speedy *= (-1)
        if self.ball.rect.colliderect(self.playerO.rect): 
            if abs(self.ball.rect.bottom -  self.playerO.rect.top) <= 20 and self.ball.speedy < 0:
                self.ball.speedx *= (-1) 
